...
class Dashboard extends Component {
  componentWillMount(){
    this.props.Fetch_Shopping_Lists(store)
  }
  renderListTableDetails(){
    if (undefined in this.props.lists){
      return(
        <h6 className="secondary-text-color">Please Create a Shopping List by Pressing the Floating "+" button.</h6>
      )
    }
    else{
    return(
  ...
  render() {
    return (
      <div>
        <Navbar text='logout' icon='input' link='/logout'/>
        <div className="container badge">
          <h6>Dashboard</h6>
          {this.renderListTableDetails()}
          <a className="right btn-floating btn-large waves-effect waves-light accent-color"><i className="material-icons">add</i></a>
        </div>
      </div>
    );
  }
